...
function ImageMap({
	...
	this.findCoordinates = ({ id, mapWidth, mapHeight, factorX, factorY }) => { fb.ShowPopupMessage('map_xxx.js: imageMap.findCoordinates() has not been set', window.FullPanelName); return [-1, -1]; }; // Must be overwritten
	// eslint-disable-next-line no-unused-vars
	...
	this.loadData = (path = this.jsonPath) => {
		if (_isFile(path)) {
			this.jsonData = [];
			const data = _jsonParseFileCheck(path, 'Tags json', window.FullPanelName, utf8);
			if (!data) { return; }
